...
protID = isInt(input('column with protein IDs (integer): '))
if args['transcFILE'] != None:
	...
	transcID = isInt(input('column of cds ID (integer): '))
gencode = ['GENCODE basic']
print('\n-- this is looking for \"'+'\" or \"'.join(gencode)+'\":')
gencodeID = isInt(input('column of GENCODE basic annotation (integer; 0=none): '))
type = ['protein_coding']
print('\n-- this is looking for \"'+'\" or \"'.join(type)+'\":')
typeID = isInt(input('column of Transcript type (integer; 0=none): '))
tsl = ['tsl1','tsl2','tsl3','tsl4','tslNA']
print('\n-- this is looking for \"'+'\" or \"'.join(tsl)+'\":')
tslID = isInt(input('column of Transcript Support Level (TSL) (integer; 0=none): '))
...
FASTADB = protdb	
if gencodeID > 0:	
	FASTADB = list(filter(protdb,INFO,2,gencode))#GENCODE basic annotation
	protdb = FASTADB
if typeID > 0:
	FASTADB = list(filter(protdb,INFO,3,type))#Transcript type
	protdb = FASTADB
if tslID > 0:
	FASTADB = list(filter(protdb,INFO,4,tsl))#Transcript Support Level (TSL)
	protdb = FASTADB
