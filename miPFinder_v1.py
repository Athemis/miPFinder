...
def isBlast(string):
    ...
    try:
        subprocess.check_output(
            ('"' + string + '"makeblastdb -dbtype prot -in testtest.txt'), shell=True
        )
    ...
    try:
        subprocess.check_output(
            (
                '"'
                + string
                + '"clustalw2 -INFILE=testtest.txt -ALIGN -TYPE=PROTEIN -OUTFILE=testtest.txt'
            ),
            shell=True,
        )
    except subprocess.CalledProcessError as e:
...
def isMafft(string):
    ...
    try:
        subprocess.check_output(
            ('"' + string + '"mafft --auto --thread -1 testtest.txt > testtest.txt'),
            shell=True,
        )
    except subprocess.CalledProcessError as e:
...
parser.add_argument(
    ...
    "--fasta",
    dest="fastadb",
    help="fasta file of all proteins, must be in the working directory! Recommended: http://www.phytozome.net, http://www.ensembl.org/, ftp://ftp.ncbi.nlm.nih.gov/refseq ->protein.faa files",
    required=True,
    type=isFile,
)
parser.add_argument(
    ...
    dest="species",
    default="NA",
    help="Prefix for output files [string] (NA)",
)
parser.add_argument(
    "-p",
    "--ProteinGeneList",
    dest="ProteinGeneList",
    help="List of identifiers of protein and genes. column1: protein_id, column2: gene_id; tab separated",
    type=isFile,
)
parser.add_argument(
    "-a",
    "--annotation",
    dest="annotationdb",
    help="Annotation file for fasta file. column1: protein_id, column2: description; tab separated",
    type=isFile,
)
parser.add_argument(
    "-m",
    "--hmmscanDB",
    dest="hmmscandb",
    help="Domain annotation file hmmscan, is created if not specified",
    type=isFile,
)
parser.add_argument(
    "-B",
    ...
    dest="blastPATH",
    default="",
    help="path/to/blast-folder/, available at ftp://ftp.ncbi.nih.gov/blast/executables/blast+/",
    type=isBlast,
)
parser.add_argument(
    "-C",
    ...
    dest="ClustalPATH",
    default="",
    help="path/to/clustalw2-folder/, available at http://www.clustal.org/clustal2/",
    type=isClustal,
)
parser.add_argument(
    "-F",
    ...
    dest="MafftPATH",
    default="",
    help="path/to/mafft-folder/, available at https://mafft.cbrc.jp",
    type=isMafft,
)
parser.add_argument(
    "-H",
    ...
    dest="hmmPATH",
    default="",
    help="path/to/hmmsearch-folder/, available at http://hmmer.org/",
    type=isHmmer,
)
parser.add_argument(
    "-S",
    "--STRING",
    dest="STRINGdb",
    help="STRING v10 detailed protein interaction file. XXXX.protein.links.detailed.v10.txt, available at http://string-db.org/cgi/download.pl",
    type=isFile,
)
parser.add_argument(
    "-d",
    "--PfamA",
    dest="PfamAdb",
    help="Pfam-database.hmm, available at ftp://ftp.ebi.ac.uk/pub/databases/Pfam/",
    type=isFile,
)
parser.add_argument(
    "-i",
    "--iPfam",
    dest="iPfamdb",
    help="iPfam-database.tsv, available at http://www.ipfam.org/. Merge homodomain and heterodomain interaction file.",
    type=isFile,
)
parser.add_argument(
    "-3",
    "--3did",
    dest="3diddb",
    help="3did_flat.tsv, available at https://3did.irbbarcelona.org. Use flat interaction file of interacting domain pairs.",
    type=isFile,
)
parser.add_argument(
    "-M",
    ...
    dest="miPmaxlength",
    default=140,
    help="Maximum length of a microProtein in aminoacids [integer] (140)",
    type=isInt,
)
parser.add_argument(
    "-A",
    "--minANCESTORlength",
    dest="ancestorminlength",
    default=250,
    help="Minimum length of a microProtein ancestor in aminoacids [integer] (250)",
    type=isInt,
)
parser.add_argument(
    "-L",
    ...
    dest="blastCUTOFF",
    default=1e-3,
    help="E-value cutoff for Blast search [0-1, float] (1e-3)",
    type=isFloat,
)
parser.add_argument(
    "-O",
    ...
    dest="overlapCUTOFF",
    default=0.6,
    help="Pfam domains: minimum overlap of a microProtein with an annotated domain; 0.5=miP matches at least half the domain, [0-1, float] (0.6)",
    type=isFloat,
)
parser.add_argument(
    "-E",
    ...
    dest="evalueCUTOFF",
    default=0.1,
    help="E-value cutoff for HMMscan and HMMsearch [0-1, float] (0.1)",
    type=isFloat,
)
parser.add_argument(
    "-V",
    ...
    dest="cvalueCUTOFF",
    default=0.05,
    help="c-Evalue cutoff for HMMscan and HMMsearch [0-1, float] (0.05)",
    type=isFloat,
)
parser.add_argument(
    "-c",
    ...
    dest="STRINGcolumn",
    default=9,
    help="Column in STRING v10 file that is compared to STRINGminscore. E.g. 9=combined score, 6=experimental, 8=textmining; [2-9, integer] (9)",
    type=isInt,
)
parser.add_argument(
    "-e",
    ...
    dest="STRINGminscore",
    default=400,
    help="Minimum score in STRING v10 file in column STRINGcolumn. E.g. 700=high confidence, 400=medium confidence; [0-1000, integer] (400)",
    type=isInt,
)
...
parser.add_argument(
    ...
    dest="IDlist",
    default=knownMIP,
    help="list of IDs that will be searched against miP candidate protein IDs and reported in the final result table [string, semicolon-separated list] (22 known Ath miPs)",
)
...
overlapCUTOFF = float(
    args["overlapCUTOFF"]
)  # minimum overlap of miP candidates with a miP ancestor domain
...
# additional settings
mostSIMILARcutoff = (
    10  # determines max miP ancestor number in alignment files and result sheet
)
gapopen = (
    " -GAPOPEN=20"  #' -GAPOPEN=30' -> gapopen penalty CLUSTALW '' for standard settings
)
...
if os.path.exists(currentPATH + "/alignment") or os.path.exists(currentPATH + "/blast"):
    ...
    if q == "Y" or q == "y":
        ...
        print(
            "WARNING: Will not delete /alignment and /blast folder, that may cause problems\n"
        )
    else:
        print("terminated")
        sys.exit()
...
def makegroups(data, ProteinGeneList):
    ...
    for array in temparray:
        ...
        if len(array) >= 2:
            ...
            tempreturn[0].append(name)
    print(
        "large groups of small proteins with more than",
        maxMIPgroupSIZE,
        "members are discarded, but saved in",
        dropfile,
    )
    return tempreturn
...
if args["STRINGdb"] != None:
    STRINGinteractions = readSTRING(
        args["STRINGdb"], int(args["STRINGcolumn"]), int(args["STRINGminscore"])
    )
if iPfamdbPATH != None:
...
if PfamAdbPATH != None and os.path.isfile(hmmscandb) == False:
    ...
    print("Running hmmscan: {}".format(hmmscanLINE))
...
blastdb = readBlastTAB(
    blastdb, fastasplit
)  # [[queryAGI],[[hitAGIs]],[[evalues]],[[starts]],[[stops]]
blastdb = evaluefilter(
    blastdb, blastCUTOFF
)  # [[queryAGI],[[hitAGIs]],[[starts]],[[stops]],[[evalues]]
blastdbALL = blastdb
...
for i in range(len(singlecopy[0])):
    ...
    for j in range(len(blastdbALL[1][INDEX])):
        if (
            len(fastadb[1][fastadb[0].index(blastdbALL[1][INDEX][j])])
            >= ancestorminlength
        ):
...
for i in range(len(singledb[0])):
    ...
    for j in range(STOP):
        name = (
            ">" + singledb[1][i][j] + "_" + singledb[2][i][j] + "-" + singledb[3][i][j]
        )
        sequence = fastadb[1][fastadb[0].index(singledb[1][i][j])][
            int(singledb[2][i][j]) - 1 : int(singledb[3][i][j])
        ]
        line += name + "\n" + sequence + "\n"
    output = open(clustalwIN, "w")
    output.write(line)
...
domtblCOMPLETE = readDOMTBL(
    ...
    cvalueCUTOFF,
    miPmaxlength,
)  # [query,[target,...],[c-evalue,...],[envfrom,...],[envto,...],querylength]
domtbl = [[], [], [], [], [], []]
...
for i in range(len(domtblCOMPLETE[0])):
    ...
    for j in range(len(domtblCOMPLETE[1][i])):
        if (
            len(fastadb[1][fastadb[0].index(domtblCOMPLETE[1][i][j])])
            >= ancestorminlength
        ):
            if tempquery == "":
...
print(
    "\nmake alignments in /alignment, use only",
    mostSIMILARcutoff,
    "best hits for alignments",
)
for i in range(len(domtbl[1])):
    ...
    clustalwLINE = (
        ...
        + " -TYPE=PROTEIN -OUTFILE="
        + clustalwOUT
    )  # might be ok, no difference so far: -GAPOPEN=3000
    subprocess.check_output(clustalwLINE, shell=True)
...
for i in range(len(outGROUP[0])):
    ...
    if annotationdb != None:
        ...
        for agi in outGROUP[1][i]:
            if (
                agi in annotation[0]
                and (annotation[1][annotation[0].index(agi)]) != agi
                and (annotation[1][annotation[0].index(agi)]) != ""
            ):
    ...
    line.append(miP)
    line.append(
        '=HYPERLINK("alignment\\' + species + "_" + outGROUP[0][i] + '.txt";"link"'
    )  # =HYPERLINK("alignment\62_3.txt";"link") #alignment
    line.append(
        '=HYPERLINK("alignment\\'
        + species
        + "_"
    ...
    line.append(
        str(
            ALIGNMENTRATING(
                outGROUP[0][i], outGROUP[1][i], outGROUP[2][i], fastadb, species
            )
        )
    )  # alignmentrating
    # min_evalue
    ...
    # domain hit
    if hmmscandb != None and os.path.isfile(hmmscandb) == True:
        domainhit = domainOverlap(
            outGROUP[0][i],
            outGROUP[2][i][:mostSIMILARcutoff],
            outGROUP[3][i][:mostSIMILARcutoff],
            outGROUP[4][i][:mostSIMILARcutoff],
            domainsDB,
            overlapCUTOFF,
            iPfamDB,
        )
...
print(
    "\nThe file",
    outfile,
    'was created. For full functionality, paste the file contents into a MS Office Excel sheet (Windows) and keep it in the same folder like the "alignment" folder.',
)
