# -*- coding: cp1252 -*-
import sys, os, datetime, subprocess, math, shutil

def splitdb(blastPATH,database,outnameSMALL,outnameBIG,blastname,smalllength,biglength,fastasplit,cpucount):#,alignments):       
		...
		if os.path.isfile('blast/'+database_basename) == False:
				# copy = ('cp '+database+' blast'+database)
				copy = 'blast/'+database_basename
		...
		if os.path.isfile('blast/'+database_basename+'.pin') == False:        
				makedb = (blastPATH+'makeblastdb -dbtype prot -in blast/'+database_basename)
				print('\n'+makedb)
		...
		if os.path.isfile(blastname) == False: 
				...
				blast = (blastPATH+'blastp -num_threads '+str(cpucount)+' -outfmt 6 -evalue 0.05 -db blast/'+database_basename+' -query '+outnameSMALL+' -out '+blastname)
				print('\n'+blast)
