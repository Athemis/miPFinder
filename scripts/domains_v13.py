...
def domains(
    hmmscandb, HMMsearchDATABASE, fastasplit, evalueCUTOFF, cvalueCUTOFF, hmmscanPATH
):
    ...
    temp = [
        ...
        [],
        [],
    ]  # [[agi],[PF],[evalue],[c-Evalue],[ali coord from],[ali coord to],[Pfam name]]
    for line in Input:
        if not line[0] == "#":
            ...
            if (
                ...
                and float(splitline[12]) <= cvalueCUTOFF
            ):  # if domain is better than cuoff values
                agi = splitline[3]
    ...
    for i, hit in enumerate(hits):
        ...
        if hit in domainsDB[0]:  # if hit has domains
            ...
            for j in range(INDEX, INDEX + domainsDB[0].count(hit)):  # for each domain
                ...
                # compare to miphit
                if (
                    len(set(mipcoordinates).intersection(hitcoordinates))
                    >= len(hitcoordinates) * overlapCUTOFF
                ):  # if overlap is greater than x%
                    ...
                    printline += (
                        ...
                        + "\t"
                        + str(
                            float(len(set(mipcoordinates).intersection(hitcoordinates)))
                            / len(hitcoordinates)
                        )
                    )  #
            ...
            if temp[0][i] not in domainhit[0]:
                ...
                if iPfamDB != None:
                    if temp[0][i][:7] in iPfamDB[0]:
                        domainhit[2].append(
                            iPfamDB[1][iPfamDB[0].index(temp[0][i][:7])]
                        )
                    else:
