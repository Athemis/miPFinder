        ...
        if len(agi) > 0 and agi[0] != "":
            ...
            if temp[0].count(agi[0]) != 1:
                print(
                    "\nWARNING:",
                    agi[0],
                    "not a unique protein identifier! Review",
                    data,
                    "!\n",
                )
...
def readiPfam(FILE):
    ...
    for line in Input:
        ...
        if len(line) == 3:
            ...
            print(
                "ERROR in iPfam file", FILE, ": expected 3 or 5 columns, not", len(line)
            )
            print("   line:", line)
...
def readDOMTBL(
    data, ProteinGeneList, fastasplit, fastadb, evalueCUTOFF, cvalueCUTOFF, miPmaxlength
):
    Input = open(data, "r")
    temp = [
        [],
    ...
    for line in Input:
        if not line[0] == "#":
            splitline = line.split()
            if (
                float(splitline[6]) <= evalueCUTOFF
                and float(splitline[11]) <= cvalueCUTOFF
            ):  # if domain is better than cutoff values
                agi = splitline[0]
                for sep in fastasplit:
                    agi = agi.replace(sep, " ")
                agi = agi.split()
    ...
    for i in range(len(temp[0])):
        if not temp[0][i] in tempreturn[0]:
            ...
            for j in range(len(target[0])):
                if (
                    target[0].count(target[0][j]) != 1
                    and target[0][j] not in targetnew[0]
    ...
    for i in range(len(temp[0])):
        ...
        for j in range(len(temp[1][i])):  # take from all non-miPs gene info
            if (
                len(fastadb[1][fastadb[0].index(temp[1][i][j])]) > miPmaxlength
            ):  # if non-miP
                genelist[0].append(temp[1][i][j])
                genelist[1].append(
                    ProteinGeneList[1][ProteinGeneList[0].index(temp[1][i][j])]
                )
